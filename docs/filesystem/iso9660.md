...
These are stored as `root_lba` and `root_size` on the `Iso9660` struct, and also cached in a static (`ROOT`). The disc cannot change underneath the kernel, so every later `probe()` reuses the cached values instead of reading the 2 KiB descriptor again — on a PIO drive that read cost as much as the data being asked for.
...
| `0x39` read part of file | Yes — offset picks the starting 2 KiB block (files are contiguous) |
| `0x3A` write part of file | No — returns `u64::MAX` |
| `0x2A` execute ELF | Yes — from the working directory when it is on the ISO, and from `/mnt/iso/bin` |
