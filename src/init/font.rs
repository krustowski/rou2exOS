...
pub static FONT_RAW: &[u8] = include_bytes!("../../poppins.psf");
...
pub static FONT: &[u8] = include_bytes!("../../poppins.psf"); 


pub unsafe fn put_pixel(x: usize, y: usize, color: u32, framebuffer: *mut u32, pitch: usize, bpp: usize) {
    let index = y * pitch + x * (bpp / 8);
    //if index + 3 < framebuffer.len() {
        framebuffer.add(index).write_volatile(color & 0xFF);
        framebuffer.add(index + 1).write_volatile((color >> 8) & 0xFF);
        framebuffer.add(index + 2).write_volatile((color >> 16) & 0xFF);
    //}
}

pub fn draw_text(s: &str, x: usize, y: usize, color: u32, framebuffer: *mut u32, pitch: usize, bpp: usize) {
    let mut cx = x;
    for ch in s.bytes() {
        draw_char(ch, x, y, framebuffer, pitch, color, FONT);
        cx += FONT_WIDTH;
    }
}
...
static PSF_FONT: &[u8] = include_bytes!("../../poppins.psf");
...
fn draw_char_psf(
    ...
    for row in 0..font.height {
        ...
        for col in 0..font.width {
            if (row_byte >> (7 - col)) & 1 != 0 {
                unsafe { put_pixel(x + col, y + row, color, framebuffer, pitch, bpp); };
            }
        }
    }
}

pub fn draw_text_psf(
    text: &str,
    font: &PsfFont,
    x: usize,
    y: usize,
